...
contract NearBlockVerification is OwnableUpgradeable, PausableUpgradeable {
    ...
    function verifyAndSaveProof(uint256[4] calldata input, uint256[8] calldata proof) public whenNotPaused {
        Layout storage $ = _getStorage();
    ...
    /// @notice Verifies a compressed proof and save the Near block hash
    /// @dev The public inputs consists of a previous block hash and a block hash.
    /// Each hash consists of 2 numbers of 256 bits
    /// @param input the public inputs for the proof
    /// @param compressedProof the compressed proof to be verified
    function verifyAndSaveCompressedProof(uint256[4] calldata input, uint256[4] calldata compressedProof)
        public
        whenNotPaused
    {
        Layout storage $ = _getStorage();
    ...
    function isProofed(uint256[2] calldata input) public view returns (bool) {
    ...
    function isProofedHash(bytes calldata hash) public view returns (bool) {
        return _getStorage().proofedHashes[hash];
    }

    /// @notice Converts an array of 256-bit integers to a hash
    /// @param array the array of 256-bit integers to convert
    /// @return the resulting hash
    function toHash(uint256[2] memory array) public pure returns (bytes memory) {
    ...
    /// @param array the array of 256-bit integers to convert
    /// @return the resulting hash
    function secondHash(uint256[4] memory array) internal pure returns (bytes memory) {
        // initialize an empty byte array to store the result
        bytes memory result = abi.encodePacked();
