...
contract NearBlockVerificationWith16Inputs {
    ...
    function toHash(uint256[8] memory array) public pure returns (bytes memory) {
        bytes memory result = abi.encodePacked();
        for (uint i = 0; i < 8; i++) {
            result = abi.encodePacked(result, uint32(array[i]));
        }

        return result;
    }

    function secondHash(uint256[16] memory array) internal pure returns (bytes memory) {
        bytes memory result = abi.encodePacked();
        for (uint i = 8; i < 16; i++) {
            result = abi.encodePacked(result, uint32(array[i]));
        }
