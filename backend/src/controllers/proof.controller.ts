...
const generateProof = async (req: Request, res: Response) => {
    ...
    try {
        ...
        // else {
        let block = await fetchBlockByHashFromNear(hash);

        let provingTask = await getProvingTask(block);

        await sendProvingTaskToQueue(provingTask);

        const hexInput = base58toHex(provingTask.previous_epoch_hash) + base58toHex(provingTask.current_hash);
