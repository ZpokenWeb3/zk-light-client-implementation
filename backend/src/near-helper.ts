...
export const fetchBlockByHashFromNear = async (hash: String) => {
    const response = await fetch('https://compatible-light-crater.near-mainnet.quiknode.pro/332447effce5b1cec9f320e24bc52cfa62882e1a/', {
        method: 'POST',
...
export const getProvingTask = async (block: any): Promise<ProvingTask> => {
    ...
    let previousEpochBlock = await fetchBlockByHeightFromNear(prevEpochBlockHeight);

    const nextBlockHeight = block.result.header.height + 1
    let nextBlock = await fetchBlockByHeightFromNear(nextBlockHeight);
