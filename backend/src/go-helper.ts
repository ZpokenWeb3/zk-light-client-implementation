...
export const goApiRequest = async (hexEncodedInputBytes: String): Promise<any> => {
    ...
    const current_block_proofData = await fs.promises.readFile(`/app/proofs/${hexEncodedInputBytes}/gnark/current_block_proof.json`, 'utf8');
    const verifier_only_circuit_dataData = await fs.promises.readFile(`/app/proofs/${hexEncodedInputBytes}/gnark/current_block_vd.json`, 'utf8');
