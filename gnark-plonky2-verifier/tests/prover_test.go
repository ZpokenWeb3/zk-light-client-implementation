...
func TestBlockCircuitProver(t *testing.T) {
	...
	if err != nil {
		fmt.Printf("failed to generate witness: %v", err)
	}
	assert.NoError(err)
	start := time.Now()
	...
	if err != nil {
		fmt.Printf("failed to marshal proof with witness: %v", err)
	}
	assert.NoError(err)
	proofFile, err := os.Create("proof_with_witness.json")
	if err != nil {
		fmt.Printf("failed to create proof_with_witness file: %v", err)
	}
	assert.NoError(err)
	_, err = proofFile.Write(jsonProofWithWitness)
	if err != nil {
		fmt.Printf("failed to write proof_with_witness file: %v", err)
	}
	assert.NoError(err)
	proofFile.Close()
